...
export async function GET() {
  ...
  const hub = await loadCompanyProfileHub(getCompanyWorkspaceUserId(session));
...
export async function PATCH(req: NextRequest) {
  ...
  if (typeof body.name === 'string') {
    await prisma.user.update({ where: { id: getCompanyWorkspaceUserId(session) }, data: { name: body.name } });
  }

  await prisma.companyProfile.upsert({
    where: { userId: getCompanyWorkspaceUserId(session) },
    create: {
