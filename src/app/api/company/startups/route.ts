...
export async function POST(req: NextRequest) {
  ...
  if (body.action === 'follow') {
    await prisma.startupFollower.upsert({
      where: { startupId_userId: { startupId: body.startupId, userId: getCompanyWorkspaceUserId(session) } },
      create: { startupId: body.startupId, userId: getCompanyWorkspaceUserId(session) },
      update: {},
    });
  } else if (body.action === 'unfollow') {
    await prisma.startupFollower.deleteMany({
      where: { startupId: body.startupId, userId: getCompanyWorkspaceUserId(session) },
    });
