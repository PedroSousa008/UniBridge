...
export default async function UniversityAcademicsPage() {
    ...
    await Promise.all([
      ...
      prisma.subjectEnrollment.findMany({
        where: {
          ...
          student: { studentProfile: { universityId } },
        },
        select: {
          studentId: true,
          subjectId: true,
  ...
  const enrollmentsByUser = new Map<
    string,
    { subjectIds: string[]; creditsRows: { grade: number | null; credits: number | null }[] }
  >();

  for (const e of enrollments) {
    const existing = enrollmentsByUser.get(e.studentId) ?? {
      subjectIds: [],
      creditsRows: [],
    };
    existing.subjectIds.push(e.subjectId);
    existing.creditsRows.push({
      grade: e.grade,
      credits: e.subject.credits,
    });
    enrollmentsByUser.set(e.studentId, existing);
  }
    ...
    <Suspense fallback={<div className="h-64 animate-pulse rounded-2xl bg-muted/40" />}>
      <UniversityAcademicsClient
        ...
        students={students.map((s) => {
          const enrollment = enrollmentsByUser.get(s.userId);
          return {
            id: s.id,
            userId: s.userId,
            ...
            completedCredits: sumCompletedCredits(enrollment?.creditsRows ?? []),
          };
        })}
