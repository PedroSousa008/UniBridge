...
export const requireStudentSubjectAccess = cache(async function requireStudentSubjectAccess(
  userId: string,
  subjectId: string
) {
...
export async function loadStudentSubjectAnnouncements(userId: string, subjectId: string) {
...
export async function loadStudentSubjectContent(userId: string, subjectId: string) {
  await requireStudentSubjectAccess(userId, subjectId);
  const [contentWeeks, contentNotes] = await Promise.all([
...
export const loadStudentSubjectHomeData = cache(async function loadStudentSubjectHomeData(
  userId: string,
  subjectId: string
) {
  ...
  const [
    ...
    prisma.subjectScheduleSlot.findMany({ where: { subjectId }, orderBy: { dayOfWeek: 'asc' }, take: 1 }),
...
export type StudentSubjectHomeData = Awaited<ReturnType<typeof loadStudentSubjectHomeData>>;

export async function loadStudentSubjectGradebookData(userId: string, subjectId: string) {
  const { enrollment, subject } = await requireStudentSubjectAccess(userId, subjectId);
  const [assignments, exams, gradeCategories] = await Promise.all([
    prisma.assignment.findMany({
      where: { subjectId },
      include: {
        gradeCategory: true,
  ...
  return { enrollment, subject, assignments, exams, gradeCategories };
}

export async function loadStudentSubjectAttendanceData(userId: string, subjectId: string) {
  const { enrollment, subject } = await requireStudentSubjectAccess(userId, subjectId);
  const attendanceSessions = await prisma.subjectAttendanceSession.findMany({
    where: { subjectId },
    include: { records: { where: { studentId: userId } } },
    orderBy: { date: 'desc' },
    take: 60,
  });
  return { enrollment, subject, attendanceSessions };
}

export async function loadStudentSubjectCareerData(userId: string, subjectId: string) {
  const { enrollment, subject } = await requireStudentSubjectAccess(userId, subjectId);
  const universityId = subject.universityId ?? subject.course?.universityId;

  const [internships, challenges, careerPaths, universityEvents] = await Promise.all([
    universityId
...
export const loadSubjectWorkspace = cache(async function loadSubjectWorkspace(
  userId: string,
  subjectId: string
) {
  const { enrollment, subject } = await requireStudentSubjectAccess(userId, subjectId);
