...
ALTER TABLE "Exam" ADD COLUMN IF NOT EXISTS "difficulty" INTEGER NOT NULL DEFAULT 3;
ALTER TABLE "Exam" ADD COLUMN IF NOT EXISTS "weight" DOUBLE PRECISION NOT NULL DEFAULT 1;
ALTER TABLE "Exam" ADD COLUMN IF NOT EXISTS "contentVolume" INTEGER NOT NULL DEFAULT 5;
...
UPDATE "Exam" e
SET "createdById" = tp."userId"
FROM "Subject" s
JOIN "TeacherProfile" tp ON tp."id" = s."teacherId"
WHERE e."subjectId" = s."id" AND e."createdById" IS NULL;

UPDATE "Exam" SET "createdById" = (SELECT "id" FROM "User" LIMIT 1) WHERE "createdById" IS NULL;
...
async function runEnsure(): Promise<boolean> {
  ...
  try {
    await prisma.$executeRawUnsafe(EXAM_SCHEMA_SQL);
    return await tableReady();
  } catch (e) {
    console.error('[ensure-exam-schema]', e);
    return false;
  }
}
